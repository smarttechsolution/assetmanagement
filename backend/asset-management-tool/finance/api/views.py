	...
	def get(self,request, *args, **kwargs):
		...
		if scheme.system_date_format == 'en':
			...
			previous_month_income = Income.objects.filter(category__water_scheme = scheme,
				...
				date__year = prev_year_month.get('prev_year')).values('category__name').annotate(total_income_amount = Sum('income_amount'))

			present_month_income = Income.objects.filter(category__water_scheme = scheme,
				date__month = month,
				date__year = year).values('category__name').annotate(total_income_amount = Sum('income_amount'))
		...
		if scheme.system_date_format == 'nep':
			...
			previous_month_income = Income.objects.filter(category__water_scheme = scheme,
				...
				date__lte = prev_year_month_range_np.get('month_end')).values('category__name').annotate(total_income_amount = Sum('income_amount'))

			present_month_income = Income.objects.filter(category__water_scheme = scheme,
				date__gte = present_year_month_range.get('month_start'),
				date__lte = present_year_month_range.get('month_end')).values('category__name').annotate(total_income_amount = Sum('income_amount'))

		previous_month = []
		for i in previous_month_income:
			income_category = default_income_category(scheme,i.get('category__name'))
			if lang == 'nep':
				previous_month.append({'income_category_name':income_category,'total_income_amount':english_to_nepali_converter(i.get('total_income_amount'))})
			else:
				previous_month.append({'income_category_name':income_category,'total_income_amount':i.get('total_income_amount')})

		present_month = []
		for i in present_month_income:
			income_category = default_income_category(scheme,i.get('category__name'))
			if lang == 'nep':
				present_month.append({'income_category_name':income_category,'total_income_amount':english_to_nepali_converter(i.get('total_income_amount'))})
			else:
				present_month.append({'income_category_name':income_category,'total_income_amount':i.get('total_income_amount')})
	...
	def get(self, request, *args, **kwargs):
		...
		if this_year:
			...
			for i in year_interval:
				if scheme.system_date_format == 'en':
					income = list(Income.objects.filter(category__water_scheme = scheme, date__gte = i.start_date, date__lte = i.end_date).order_by('date__year', 'date__month').values('date__year', 'date__month').annotate(total_amount = Sum('income_amount')))
					expense = list(Expenditure.objects.filter(category__water_scheme = scheme,date__gte = i.start_date, date__lte = i.end_date).order_by('date__year', 'date__month').values('date__year', 'date__month').annotate(total_amount = Sum('income_amount')))
					income_year_month = []
					for inc in income:
...
class IncomeDistributionReportByCategory(APIView):
	...
	def get(self, request, *args, **kwargs):
		...
		income_this_year = list(Income.objects.filter(category__water_scheme = scheme, date__gte = this_year_interval.start_date, date__lte = this_year_interval.end_date).values('category__name').annotate(total_amount = Sum('income_amount')))
		income_all_time = list(Income.objects.filter(category__water_scheme = scheme).values('category__name').annotate(total_amount = Sum('income_amount')))
...
class ExpenseDistributionReportByCategory(APIView):
	...
	def get(self, request, *args, **kwargs):
		...
		expense_this_year = list(Expenditure.objects.filter(category__water_scheme = scheme, date__gte = this_year_interval.start_date, date__lte = this_year_interval.end_date).values('category__name').annotate(total_amount = Sum('income_amount')))
		expense_all_time = list(Expenditure.objects.filter(category__water_scheme =  scheme).values('category__name').annotate(total_amount = Sum('income_amount')))
